...
lexer_rs_content = File.read './src/lexer/lexer.rs'
...
lexer_rs_content.gsub! "!write token tables matching\n", """
...
#     emit T_LBRACK_
# 
# lexer_rs_content.gsub!(/emit (\w+);/) do |match| "emit #$1, 0, 0;" end
# lexer_rs_content.gsub!(/emit (\w+), (\d+), (\d+);/) do |match|
#   variant = $1
#   start_offset = $2
#   end_offset = $3
...
# emit_table KEYWORD
lexer_rs_content.gsub!(/!emit_table (\w+);/) do |match|
  table = $1
...
File.open('./src/lexer/lexer.rs', 'w') { |f| f.write lexer_rs_content }
